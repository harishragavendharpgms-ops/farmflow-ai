                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {activeOrders.map(order => (
                    <div key={order.id} style={{ padding: '12px', borderLeft: `4px solid ${order.status === 'Approved' ? '#4caf50' : order.status === 'Rejected' ? '#f44336' : '#2196f3'}`, backgroundColor: '#f9f9f9', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                      <div><strong style={{ display: 'block', fontSize: '16px', color: '#2c3e50' }}>{order.item}</strong><span style={{ color: '#555', fontSize: '13px' }}>{order.quantity} Units • {order.datetime}</span><span style={{ display: 'block', color: '#777', fontSize: '12px', marginTop: '3px' }}>📍 {order.location}</span></div>
        ...
        {activeTab === 'procurement' && orderingItem && (
          <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', maxWidth: '500px', margin: '0 auto' }}>
            ...
            <form onSubmit={submitOrder} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <input type="text" required placeholder={l.farmLocation} value={orderDetails.location} onChange={(e) => setOrderDetails({...orderDetails, location: e.target.value})} style={{ padding: '10px', border: '1px solid #ddd', borderRadius: '6px', fontSize: '14px' }} />
