...
const Dashboard = () => {
  ...
  // Fetch active officer zones created by the admin from Firebase
  useEffect(() => {
    const fetchOfficerZones = async () => {
      try {
        const q = query(collection(db, 'users'), where('role', '==', 'officer'));
        const querySnapshot = await getDocs(q);
        const zones = querySnapshot.docs.map(doc => doc.data().zone).filter(Boolean);
  ...
  const submitOrder = async (e) => {
    ...
    try {
      ...
      alert(`Success! Application submitted to the officer at ${orderDetails.zone}.`);
            ...
            <form onSubmit={submitOrder} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <select required value={orderDetails.zone} onChange={(e) => setOrderDetails({...orderDetails, zone: e.target.value})} style={{ padding: '10px', border: '1px solid #ddd', borderRadius: '6px', fontSize: '14px' }}>
                ...
                {availableZones.length === 0 ? (
                  <option value="" disabled>No active officer locations found. Admin must create an officer first.</option>
                ) : (
